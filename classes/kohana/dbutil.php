 ...
 * This is a port of DBUtil class from Fuel into Kohana 3.1
 *
 * @package    Kohana-DBUtil
 * @version    1.0
 * @author     Fuel Development Team
 * @author     Michał Matyas 
 * @license    MIT License
 * @copyright  2010 - 2011 Fuel Development Team
 * @copyright  Michał Matyas
 * @link       http://fuelphp.com
 * @link       http://github.com/d4rky-pl/kohana-dbutil
 */
...
class Kohana_DBUtil {
	...
	public static function create_database($database, $charset = null)
	{
		...
		return DB::query(NULL, 'CREATE DATABASE '.self::quote_identifier($database).$charset)->execute();
	}
	...
	{
		return DB::query(NULL, 'DROP DATABASE '.self::quote_identifier($database))->execute();
	}
	...
	{
		return DB::query(NULL, 'DROP TABLE IF EXISTS '.self::quote_identifier(self::db()->table_prefix().$table))->execute();
	}
	...
	{
		return DB::query(NULL, 'RENAME TABLE '.self::quote_identifier(self::db()->table_prefix().$table).' TO '.self::quote_identifier(self::db()->table_prefix().$new_table_name))->execute();
	}
	...
	public static function create_table($table, $fields, $primary_keys = array(), $if_not_exists = true, $engine = false, $charset = null)
	{
		...
		$sql .= self::quote_identifier(self::db()->table_prefix().$table).' (';
		$sql .= static::process_fields($fields);
		if ( ! empty($primary_keys))
		{
			$key_name = self::quote_identifier(implode('_', $primary_keys));
			$primary_keys = self::quote_identifier($primary_keys);
	...
	{
		$sql = 'ALTER TABLE '.self::quote_identifier(self::db()->table_prefix().$table).' ';
		if ($type === 'DROP')
		{
			if( ! is_array($fields))
			{
			...
			$fields = array_map(function($field){
				return 'DROP '.self::quote_identifier($field);
			}, $fields);
			$sql .= implode(', ', $fields);
		} else {
		  $sql .= $type.' ';
	...
	protected static function process_fields($fields)
	{
		...
		foreach ($fields as $field => $attr)
		{
			...
			$sql .= self::quote_identifier($field);
			$sql .= array_key_exists('NAME', $attr) ? ' '.self::quote_identifier($attr['NAME']).' ' : '';
			...
			$sql .= array_key_exists('DEFAULT', $attr) ? ' DEFAULT '. (($attr['DEFAULT'] instanceof Database_Expression) ? $attr['DEFAULT']  : self::db()->escape($attr['DEFAULT'])) : '';
	...
	{
		$charset or $charset = Kohana::config('database.'.Database::$default.'.charset');
		if(empty($charset))
		{
			return '';
		}
	...
	{
		return DB::query(NULL, 'TRUNCATE TABLE '.self::quote_identifier(self::db()->table_prefix().$table))->execute();
	}
	...
	{
		$result = DB::query(NULL, $operation.' '.self::quote_identifier(self::db()->table_prefix().$table))->execute();
		...
		if($type === 'error')
		{
			Kohana::$log->add(Log::ERROR, 'Table: '.$table.', Operation: '.$operation.', Message: '.$result->get('Msg_text'). ' DBUtil::table_maintenance');
		}
		else
		{
			Kohana::$log->add(Log::INFO, ucfirst($type). ' - Table: '.$table.', Operation: '.$operation.', Message: '.$result->get('Msg_text'). ' DBUtil::table_maintenance');
		}
		return false;
	}
	
	/*
	 * The Database_Connection might not have fired yet.
	 *
	 */
	...
	/*
	 * Helper method, shortcut for Database::instance()
	 * 
	...
	public static function quote_identifier($string, $db = null)
	{
		...
		return self::db($db)->quote_identifier($string);
	}
